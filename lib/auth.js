...
var User = require('../models/user'),
    ...
    accessHelper = require('./accessHelper');
...
//                    message: {errmsg: 'Username not found', errcode: '100'}
            ...
            if (!user.passwordMatches(password)) {
                return done(null, false, {
//                    message: {errmsg: 'Incorrect Password', errcode: '101'}
...
exports.injectUser = function() {
    return function injectUser(req, res, next) {
        if (req.isAuthenticated()) {
            res.locals.user = req.user;
            res.locals.accessArray = accessHelper.getAccessArray(req.user.role);
        }
        next();
    };
};